...
class DigraphBuilder<TChild>(IEqualityComparer<TChild> childEqualityComparer)
{
    ...
    public Digraph<TChild> Build(CancellationToken ct = default)
    {
        ...
        foreach (var unresolvedNode in _nodes.Values)
        {
            ...
            if (unresolvedNode.ReferenceIds is [])
            {
    ...
    public override String ToString()
    {
        ...
        foreach (var node in _nodes.Values)
        {
            ...
            {
                var prefix = node.ReferenceIds is [] && i++ == node.Children.Count - 1
                    ? "\u2514\u2500"
                    : "\u251c\u2500";
                builder.AppendLine($"{prefix}{child}");
            }

            for (var j = 0; j < node.ReferenceIds.Length; j++)
            {
                var reference = node.ReferenceIds[j];
                var prefix = j == node.ReferenceIds.Length - 1
                    ? "\u2514>"
                    : "\u251c>";
                builder.AppendLine($"{prefix}{reference}");
            }
        }
